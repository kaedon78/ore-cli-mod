...
impl Miner {
    pub async fn claim_by_number(&self, keypair_number: u64, beneficiary: Option<String>, amount: Option<f64>) {
        self.claim(&self.signer_by_number(keypair_number), beneficiary, amount).await
    }

    pub async fn claim(&self, signer: &Keypair, beneficiary: Option<String>, amount: Option<f64>,) {
        ...
        let beneficiary = match beneficiary {
            ...
            None => self.initialize_ata(&signer).await,
        };
        let proof = get_proof(&self.rpc_client, signer.pubkey()).await;
        let rewardtotal = (proof.claimable_rewards as f64) / 10f64.powf(ore::TOKEN_DECIMALS as f64);
        if rewardtotal == 0.0 {
            println!("Nothing to claim for this address");
            return;
        }
        else {
            println!("{} ORE to claim on this address", rewardtotal);
        }
        ...
        println!("Submitting claim transaction for {}...", pubkey.to_string());
        match self
            .send_and_confirm(&[cu_limit_ix, cu_price_ix, ix], false, false, vec![&signer])
            .await
        {
            Ok(sig) => {
